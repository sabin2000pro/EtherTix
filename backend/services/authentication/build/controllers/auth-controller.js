...
const registerUser = (request, response, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (password !== passwordConfirm) {
        return next(new error_response_1.ErrorResponse(`Password confirmation error. Please check passwords`, http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    const existingUser = yield user_model_1.User.findOne({ email }); // Find an existing user
    if (existingUser) {
        return next(new error_response_1.ErrorResponse("User already created", http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
...
const loginUser = (request, response, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!email || !password) {
        return next(new error_response_1.ErrorResponse(`Missing e-mail address or password. Check entries`, http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
    const user = yield user_model_1.User.findOne({ email });
    if (!user) {
        return next(new error_response_1.ErrorResponse(`Could not find that user`, http_status_codes_1.StatusCodes.NOT_FOUND));
    }
    // Compare user passwords before logging in
    const matchPasswords = yield user.comparePasswords(password);
    if (!matchPasswords) {
        return next(new error_response_1.ErrorResponse(`Passwords do not match. Please try again`, http_status_codes_1.StatusCodes.BAD_REQUEST));
    }
